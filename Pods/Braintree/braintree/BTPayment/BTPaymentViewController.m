...
- (void)viewDidLoad {
    ...
    [_paymentFormView setUKSupportEnabled:_UKSupportEnabled];
    ...
    _submitButton.accessibilityLabel = @"Submit New Card";
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_hasPaymentMethods && indexPath.section == 0) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0f) {
            return 94 - 6; //-6 to get nice-sized padding between VTCardView and "Submit New Card" button
        } else {
